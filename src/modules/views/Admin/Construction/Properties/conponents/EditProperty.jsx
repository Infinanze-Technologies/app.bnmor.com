...
const EditProperty = (props) => {
  ...
  useEffect(() => {
    try {
      ...
      form.setFieldsValue({
        ...
        // Set the selected category to trigger subcategory and status loading
  ...
  const onFinish = async values => {
    ...
    if (shouldShowField('price')) {
      data.price = parseFloat(values.price) || 0;
    }
    ...
    updateRequest(URL_UPDATE_PROPERTIES, record.uuid, { ...data }, jwt)
    .then(async (res) => {
  ...
  return (
    <div style={{ 
      ...
      <Card
        title={<span style={{ fontSize: 24, fontWeight: 700, color: "#2a3f54" }}>Edit Property</span>}
        bordered={false}
