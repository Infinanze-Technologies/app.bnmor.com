...
const StatusView = ({ record, setIsModalVisible }) => {
  const statuses = record?.statuses || [];
  const activeStatuses = statuses.filter(status => status.status);
  const inactiveStatuses = statuses.filter(status => !status.status);
        ...
        <Col xs={24} sm={8}>
          <Card size="small" style={{ textAlign: 'center', background: '#f0f9ff', border: '1px solid #0ea5e9' }}>
            <Statistic
              ...
              prefix={<InfoCircleOutlined style={{ color: '#0ea5e9' }} />}
        ...
        locale={{
          emptyText: (
            <div style={{ 
              ...
              <InfoCircleOutlined style={{ fontSize: '48px', color: '#9ca3af', marginBottom: '16px' }} />
              <Title level={4} style={{ color: '#6b7280', margin: 0 }}>
                No Statuses Found
              </Title>
              <Text type="secondary">
                This category doesn't have any statuses defined yet.
              </Text>
            </div>
          )
        }}
