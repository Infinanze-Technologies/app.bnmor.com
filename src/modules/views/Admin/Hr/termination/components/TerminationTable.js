...
// console.log('====================================');
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddTermination setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} forceRefetch={forceRefetch} qryEmployeeData={qryEmployeeData} qryAttrData={qryAttrData}/>)
      }
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_TERMINATION, record?.termination_id, jwt)
      .then(async(res) => {
