...
import {URL_BRANCH_STATUS,URL_DELETE_AWARD,URL_DELETE_BRANCH, URL_DELETE_DEPARTMENT, URL_DELETE_LEAVE, URL_DELETE_RESIGNATION, URL_DELETE_TERMINATION, URL_DELETE_TIMESHEET } from "@/config/api-paths";
...
// console.log('====================================');
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddResignation setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} forceRefetch={forceRefetch} qryEmployeeData={qryEmployeeData} qryAttrData={qryAttrData}/>)
      }
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_RESIGNATION, record?.resignation_id, jwt)
      .then(async(res) => {
