...
import {URL_BRANCH_STATUS,URL_DELETE_BRANCH, URL_DELETE_DEPARTMENT, URL_DELETE_TIMESHEET } from "@/config/api-paths";
...
const TimeSheetTable = (props) => {
  ...
  let {  jwt,TimesheetDataObject,QryEmployeeDataObject } = props;
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddTimeSheet setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} forceRefetch={forceRefetch} qryEmployeeData={qryEmployeeData}/>)
      }
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_TIMESHEET, record?.timesheet_id, jwt)
      .then(async(res) => {
