...
const { Option } = Select;
import { URL_UPDATE_ACCOUNTLIST, URL_UPDATE_BRANCH, URL_UPDATE_DEPARTMENT, URL_UPDATE_PAYEE, URL_UPDATE_TIMESHEET } from "@/config/api-paths";
...
const EditPayees = (props) => {
  ...
  const onFinish = (values) => {
    ...
    updateRequest(URL_UPDATE_PAYEE, record?.payee_id, { ...data }, jwt)
      .then((res) => {
  ...
  return (
    <>
       <Form onFinish={onFinish} form={form} name="basic" layout="vertical">
        <Card type="inner" title="Edit Payees" style={{ marginBottom: 24, borderRadius: 8 }}>
        <Row gutter={16}>
