...
import EditPayees from "./EditPayees";
import {URL_BRANCH_STATUS,URL_DELETE_ACCOUNTLIST,URL_DELETE_BRANCH, URL_DELETE_DEPARTMENT, URL_DELETE_PAYEE, URL_DELETE_TIMESHEET } from "@/config/api-paths";
...
import AddPayees from "./AddPayees";
...
const PayeesTable = (props) => {
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [modalTitle, setModalTitle] = useState();
  const [modalWidth, setModalWidth] = useState();
  const [modalContent, setModalContent] = useState("");
  const { handleRequestError, handleRequestResponse } = useHandleResponse()
  let {  jwt,PayeeDataObject,QryBranchDataObject } = props;
  ...
  let qryData = PayeeDataObject?.data
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddPayees setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} qryBranchData={qryBranchData}/>)
      }
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_PAYEE, record?.payee_id, jwt)
      .then((res) => {
