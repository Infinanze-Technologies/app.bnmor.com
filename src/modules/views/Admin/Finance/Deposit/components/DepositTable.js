...
import {URL_BRANCH_ACCOUNT_LIST,URL_DELETE_ACCOUNTLIST,URL_DELETE_BRANCH, URL_DELETE_DEPARTMENT, URL_DELETE_DEPOSIT, URL_DELETE_TIMESHEET } from "@/config/api-paths";
...
const DepositTable = (props) => {
  ...
  let {  jwt,AccountDepositDataObject,QryBranchDataObject,AttributesDataObject,PaymentTypeAttributesDataObject,CoaForDepositsData } = props;
  ...
  } = AccountDepositDataObject;

  let qryData = AccountDepositDataObject?.data
  ...
  }, [AccountDepositDataObject,qryBranchData]);
  ...
  const columns = [
    ...
    {
      title: 'CATEGORY',
      dataIndex: 'income_type_id',
      align: "center",
       render: (text,record,index,boolean)=> record?.income_type?.name
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(800);
      setModalContent(<AddDeposit setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} qryBranchData={qryBranchData} qryAttrData={qryAttrData} qryAttrPaymentType={qryAttrPaymentType} CoaForDepositsData={CoaForDepositsData}/>)
      }
      ...
      setModalContent(
        <EditDeposit
          ...
          CoaForDepositsData={CoaForDepositsData}
        />
      );
    } else {
      return false;
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_DEPOSIT, record?.deposit_id, jwt)
      .then((res) => {
