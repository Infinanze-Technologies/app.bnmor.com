...
// Config
import { URL_BRANCH_ACCOUNT_LIST, URL_BRANCH_PAYEE, URL_UPDATE_EXPENSES } from "@/config/api-paths";
...
const EditExpense = (props) => {
  ...
  useEffect(() => {
    if (record) {
      try {
        ...
        fetchPayeesByBranchId(record?.branch_id);
      } catch (error) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      const response = await updateRequest(URL_UPDATE_EXPENSES, record?.expense_id, data, jwt);
  ...
  return (
    <Form onFinish={onFinish} form={form} name="basic" layout="vertical">
      <Card type="inner" title="Edit Expense" style={{ marginBottom: 24, borderRadius: 8 }}>
        <Row gutter={16}>
          {/* Branch Selection */}
