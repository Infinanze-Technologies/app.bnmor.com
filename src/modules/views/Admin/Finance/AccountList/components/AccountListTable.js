...
import EditAccountList from "./EditAccountList";
...
const AccountListTable = (props) => {
  ...
  let {  jwt,AccountListDataObject,QryBranchDataObject } = props;
  ...
  } = AccountListDataObject;

  let qryData = AccountListDataObject?.data
  ...
  }, [AccountListDataObject,qryBranchData]);
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddAccountList setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} qryBranchData={qryBranchData}/>)
      }
