...
import EditBorrowerStatus from "./EditBorrowerStatus";
import DeleteBorrower from "./DeleteBorrower";
import ViewBorrowerFiles from "./ViewBorrowerFiles";
...
const BorrowerTable = (props) => {
  ...
  let { BorrowerDataObject, QryBranchDataObject, jwt,RoleDataObject,setLoading,loading } = props;
  ...
  let qry_data = BorrowerDataObject?.data?.data || BorrowerDataObject?.data?.items || BorrowerDataObject?.data || []
...
const onPageChange = (page, pageSize) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text,record,index)=> (
        <>
          {formatDateHuman(record?.date_of_registration)}
        </>
      )
    },
    // {
  ...
  return (
    <>
            ...
            <UserTableStyleWrapper>
              <TableWrapper style={{
                ...
                <Table
                  ...
                  locale={{
                    emptyText: qry_data?.length === 0 ? 'No borrower data found' : 'No data'
                  }}
                  rowKey="id"
                  style={{
                    background: '#ffffff'
      ...
      <ViewBorrowerFiles
        ...
        forceRefetch={forceRefetch}
        onFileUpdate={() => {
          // Refresh the borrower data after file update
          forceRefetch();
        }}
      />
    </>
