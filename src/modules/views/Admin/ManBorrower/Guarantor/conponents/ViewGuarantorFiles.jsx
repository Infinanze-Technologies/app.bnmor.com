...
import EditGuarantorFile from './EditGuarantorFile';

const { Title, Text } = Typography;

const ViewGuarantorFiles = ({ record, visible, onCancel, jwt, onFileUpdate }) => {
    ...
    <Modal
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span>Guarantor Files - {record?.fullname || 'Unknown'}</span>
        </div>
      }
      open={visible}
      onCancel={onCancel}
      ...
      <Spin spinning={loading}>
        ...
        {!record?.files?.profile_img && !record?.files?.front_id && !record?.files?.back_id && (
          <div style={{
            ...
            <Text type="secondary">This guarantor has not uploaded any files yet.</Text>
          </div>
        )}
      </Spin>
...
export default ViewGuarantorFiles;
