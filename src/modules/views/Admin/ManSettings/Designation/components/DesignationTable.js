...
import {URL_BRANCH_STATUS,URL_DELETE_BRANCH, URL_DELETE_DEPARTMENT, URL_DELETE_DESIGNATION } from "@/config/api-paths";
...
const DesignationTable = (props) => {
  ...
  let {  jwt,DesignationDataObject,QryDepartmentDataObject } = props;
  ...
  } = DesignationDataObject;

  let qryData = DesignationDataObject?.data

  let qryDepartmentData = QryDepartmentDataObject?.data
  ...
  }, [DesignationDataObject,QryDepartmentDataObject]);
 ...
 />
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalWidth(500);
      setModalContent(<AddDesignation setIsModalVisible={setIsModalVisible} jwt={jwt} refetch={refetch} qryDepartmentData={qryDepartmentData}/>)
      }
      ...
      setModalContent(
        <EditDesignation
          ...
          record={record}
          refetch={refetch}
          qryDepartmentData={qryDepartmentData}
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_DESIGNATION, record?.id, jwt)
        .then(async (res) => {
