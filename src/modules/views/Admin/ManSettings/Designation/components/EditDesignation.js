...
import { URL_UPDATE_DESIGNATION } from "@/config/api-paths";
...
const EditDesignation = (props) => {
  const { jwt, setIsModalVisible, record, refetch, qryDepartmentData,forceRefetch } = props;
  ...
  const onFinish = (values) => {
    ...
    updateRequest(URL_UPDATE_DESIGNATION, record?.id, { ...values }, jwt)
      .then(async (res) => {
  ...
  return (
      <Form
        ...
        layout="vertical"
      >
        <Card type="inner" title="Edit Designation" style={{ marginBottom: 24, borderRadius: 8 }}>
      <Row gutter={16}>
          <Col span={24} md={24}>
          <CustomSelect
              label="Department"
