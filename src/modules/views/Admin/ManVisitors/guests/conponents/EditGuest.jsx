...
import { URL_UPDATE_VISITOR_GUESTS, URL_GET_Qry_BRANCH, URL_GET_ACTIVE_ORGANIZATIONS } from "@/config/api-paths";
...
const EditGuest = (props) => {
  let { jwt, record, refetch, QryBranchDataObject, setpage, setIsModalVisible, refreshOrganizations, refreshAllData } = props;
  ...
  // Fetch active organizations for organization selection
  ...
  useEffect(() => {
    if (record) {
      // Set initial values
      setGuestType(record?.type || 'Individual');
  ...
  const onFinish = (values) => {
    try {
      ...
      updateRequest(URL_UPDATE_VISITOR_GUESTS, record?.guest_id, guestData, jwt)
        .then((res) => {
  ...
  return (
    <div style={{ 
      ...
      <Card
        title={<span style={{ fontSize: 24, fontWeight: 700, color: "#2a3f54" }}>Edit Guest</span>}
        bordered={false}
