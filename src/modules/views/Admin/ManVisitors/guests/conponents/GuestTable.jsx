...
import AddEntry from "./ViewEntry";
import GuestHistoryView from "./GuestHistoryView";
import { URL_DELETE_VISITOR_GUESTS, URL_VISITOR_GUESTS_STATUS } from "@/config/api-paths";
...
const GuestTable = (props) => {
  ...
  let { GuestDataObject, QryBranchDataObject, setLoading, loading ,jwt, ActiveOrganizationsDataObject, HostsDataObject, DepartmentsDataObject, refreshOrganizations,refreshAllData} = props;
  ...
  let qry_data = GuestDataObject?.data || []
  // console.log('====================================');    
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Tag 
          color={record.type === 'Individual' ? 'blue' : 'green'}
          style={{ 
            fontWeight: '500',
            padding: '4px 8px',
            borderRadius: '4px'
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_VISITOR_GUESTS, record?.id, jwt)
      .then((res) => {
  ...
  const menu = (record) => (
    ...
    >
      ...
      >
        <a onClick={() => showModal("history", record)} style={{ color: '#4D4D4D' }}>View Entries</a>
      </Menu.Item>
      {/* <Menu.Item key="2" style={{
        color: '#4D4D4D',
        fontWeight: '500',
      ...
      >
        <a onClick={() => showModal("addEntry", record)} style={{ color: '#4D4D4D' }}>Add Entry</a>
      </Menu.Item> */}
            ...
            <UserTableStyleWrapper>
              <TableWrapper style={{
                ...
                <Table
                  ...
                  locale={{
                    emptyText: qry_data?.length === 0 ? 'No guest data found' : 'No data'
                  }}
                  rowKey="id"
                  style={{
                    background: '#ffffff'
