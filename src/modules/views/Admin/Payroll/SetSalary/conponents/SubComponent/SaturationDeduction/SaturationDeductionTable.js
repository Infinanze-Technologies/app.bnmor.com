...
import { URL_DELETE_ALLOWANCE, URL_DELETE_DEDUCTION, URL_GET_ALLOWANCE, URL_GET_DEDUCTION, URL_GET_QRY_ATTRIBUTE, URL_GET_SALARY } from "@/config/api-paths";
import ModalComponent from "@/components/ModalComponent";
import EditSaturationDeduction from "./EditSaturationDeduction";
...
const AllowanceTable = (props) => {
  ...
  const EmpDeductionObject = useFetchQuery({
    url: URL_GET_DEDUCTION+`/${employee_id}`,
    jwt: jwt,
  ...
  const showModal = (value, record) => {
    ...
    if (value == "add") {
      ...
      setModalTitle(<AddSaturationDeductionTitle/>);
      setModalWidth(500);
      setModalContent(<AddSaturationDeduction setIsModalVisible={setIsModalVisible} jwt={jwt} employee_id={employee_id} refetch={EmpDeductionObject?.refetch} qryAttrPalyslipType={qryAttrPalyslipType}/>)
      }

    else if (value == "edit") {
      setIsModalVisible(true);
      setModalTitle(<EditSaturationDeductionTitle />);
      setModalWidth(800);
      setModalContent(
        <EditSaturationDeduction
          setIsModalVisible={setIsModalVisible}
          jwt={jwt}
          record={record}
          refetch={EmpDeductionObject?.refetch}
  ...
  const EditSaturationDeductionTitle = () => (
    <div className="flex flex-wrap" style={{ width: 700 }}>
      <h6>Edit Saturation Deduction</h6>
    </div>
  );

  const AddSaturationDeductionTitle = () => (
    <div className="flex flex-wrap" style={{ width: 700}}>
    <h6>Create New Saturation Deduction</h6>
  ...
  const handleDelete = (record) => {
    deleteRequest(URL_DELETE_DEDUCTION, record?.deduction_id, jwt)
      .then((res) => {
        handleRequestResponse(res);
        EmpDeductionObject?.refetch();
      })
      .catch((err) => {
