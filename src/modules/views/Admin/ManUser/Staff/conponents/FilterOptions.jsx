...
const FilterOptions = (props) => {
  const { setfilterUserData, QryBranchDataObject, RoleDataObject, DepartmentsDataObject, ActiveDesignationDataObject } = props;
  ...
  const roleOptions = RoleDataObject?.data || [];
  const branchOptions = QryBranchDataObject?.data || [];
  const departmentOptions = DepartmentsDataObject?.data || [];
  const designationOptions = ActiveDesignationDataObject?.data || [];
  ...
  const applyFilters = () => {
    ...
    if (selectedRole !== 'ALL') {
      apiFilter += `role_id=${selectedRole}&`;
    }
    ...
    if (selectedDesignation !== 'ALL') {
      apiFilter += `designation_id=${selectedDesignation}&`;
    }
  ...
  return (
    <>
      ...
      />
          ...
          <Col xs={24} sm={6} md={6}>
            ...
            >
              <Option value="ALL">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '16px' }}>👤</span>
                  <span style={{ fontWeight: '500' }}>All Roles</span>
                </div>
              </Option>
          ...
          <Col xs={24} sm={6} md={6}>
            ...
            >
              <Option value="ALL">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '16px' }}>💼</span>
                  <span style={{ fontWeight: '500' }}>All Designations</span>
                </div>
              </Option>
        ...
        {(searchText || selectedBranch !== 'ALL' || selectedRole !== 'ALL' || selectedDepartment !== 'ALL' || selectedDesignation !== 'ALL' || selectedDateRange !== 'ALL') && (
          <div style={{ 
            marginTop: '24px', 
            padding: '16px',
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              ...
              {selectedRole !== 'ALL' && (
                ...
                >
                  👤 Role: {roleOptions.find(r => r.id === selectedRole)?.name || selectedRole}
                </Tag>
              )}
              {selectedDepartment !== 'ALL' && (
                ...
                >
                  🏢 Department: {departmentOptions.find(d => d.id === selectedDepartment)?.name || selectedDepartment}
                </Tag>
              )}
              {selectedDesignation !== 'ALL' && (
                <Tag 
                  closable 
                  onClose={() => setSelectedDesignation('ALL')}
                  style={{ 
                    margin: 0,
                    borderRadius: '20px',
                    padding: '6px 12px',
                ...
                >
                  💼 Designation: {designationOptions.find(d => d.id === selectedDesignation)?.name || selectedDesignation}
                </Tag>
              )}
      ...
      >
        <div style={{ marginBottom: '20px' }}>
          <p style={{ color: '#4D4D4D', marginBottom: '16px', fontWeight: '500' }}>
            Choose your custom date range for filtering staff
          </p>
