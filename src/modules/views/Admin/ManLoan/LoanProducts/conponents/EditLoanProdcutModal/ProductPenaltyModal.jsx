...
import { URL_UPDATE_LOAN_PRODUCT_PENALTY_DETAILS } from "@/config/api-paths";
import useHandleResponse from "@/hooks/useHandleResponse";
import { BUTTON_CONFIGS } from "@/utils/buttonStyles";
const { Option } = Select;

const ProductPenaltyModal = (props) => {
  let jwt = props?.jwt;
  ...
  useEffect(() => {
    if (record) {
      ...
      setPenaltyType(record.late_repayment_penalty?.penalty_type || 'Percentage');
      
      form.setFieldsValue({
        late_repayment_penalty: {
          penalty_type: record.late_repayment_penalty?.penalty_type,
          calculate_penalty_on: record.late_repayment_penalty?.calculate_penalty_on,
          penalty_percentage: record.late_repayment_penalty?.penalty_percentage,
          penalty_amount: record.late_repayment_penalty?.penalty_amount,
          grace_period: record.late_repayment_penalty?.grace_period,
          recurring_penalty: record.late_repayment_penalty?.recurring_penalty
        }
      });
    }
  }, [record, form]);
  ...
  const onFinish = async values => {
    ...
    try {
      ...
      await updateRequest(URL_UPDATE_LOAN_PRODUCT_PENALTY_DETAILS,record.product_id, {...data}, jwt)
      .then((res) => {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: 24 }}>
      <Card
        title={<span style={{ fontSize: 20, fontWeight: 700, color: "#2a3f54" }}>Edit Product Penalty</span>}
        bordered={false}
...
export default ProductPenaltyModal;
