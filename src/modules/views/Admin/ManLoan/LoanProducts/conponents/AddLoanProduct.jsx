...
import { URL_ADD_LOAN_PRODUCT, URL_GET_CASH_FUNDING_ACCOUNTS, URL_GET_COA_FOR_LOANS, URL_GET_FUNDING_BRANCHES } from "@/config/api-paths";
import useHandleResponse from "@/hooks/useHandleResponse";
import useFetchQuery from "@/hooks/ReactQuery/useFetchQuery";
import Link from "next/link";
import { LOAN_PRODUCTS_PAGE } from "@/config/page-routes";
...
const AddLoanProduct = (props) => {
  ...
  // Update form values when data is loaded to select first options by default
  useEffect(() => {
    if (CashFundingAccountsData?.data && CashFundingAccountsData.data.length > 0) {
      const currentFundingAccount = form.getFieldValue(['accounts', 'funding_account']);
      if (!currentFundingAccount) {
        const firstAccountId = CashFundingAccountsData.data[0].id;
        form.setFieldsValue({
          accounts: {
            ...form.getFieldsValue().accounts,
            funding_account: firstAccountId
          }
        });
        setSelectedFundingAccount(firstAccountId);
      }
    }
  }, [CashFundingAccountsData?.data, form]);

  useEffect(() => {
    if (CoaForLoansData?.data && CoaForLoansData.data.length > 0) {
      const currentValues = form.getFieldsValue().accounts || {};
      const updates = {};

      // Set default values for each account type if not already set
      if (!currentValues.loans_receivable_account) {
        const loansReceivable = CoaForLoansData.data.find(acc => acc.acc_name === "Loans Receivable");
        updates.loans_receivable_account = loansReceivable?.id || CoaForLoansData.data[0].id;
      }

      if (!currentValues.interest_income_account) {
        const interestIncome = CoaForLoansData.data.find(acc => acc.acc_name === "Interest Income");
        updates.interest_income_account = interestIncome?.id || CoaForLoansData.data[0].id;
      }

      if (!currentValues.fees_income_account) {
        const feesIncome = CoaForLoansData.data.find(acc => acc.acc_name === "Loan Fee Income");
        updates.fees_income_account = feesIncome?.id || CoaForLoansData.data[0].id;
      }

      if (!currentValues.penalty_income_account) {
        const penaltyIncome = CoaForLoansData.data.find(acc => acc.acc_name === "Penalty Income");
        updates.penalty_income_account = penaltyIncome?.id || CoaForLoansData.data[0].id;
      }

      if (!currentValues.overpayment_account) {
        const overpayment = CoaForLoansData.data.find(acc => acc.acc_name === "Overpayment");
        updates.overpayment_account = overpayment?.id || CoaForLoansData.data[0].id;
      }
  ...
  // Handle funding account change and update branch funding
  useEffect(() => {
    if (selectedFundingAccount) {
      // Clear the branch funding field when funding account changes
      form.setFieldsValue({
        accounts: {
          ...form.getFieldsValue().accounts,
  ...
  const handleFundingAccountChange = (value) => {
    ...
    // Reset funding branch when funding account changes
    form.setFieldsValue({
      accounts: {
        ...form.getFieldsValue().accounts,
        funding_branch_id: undefined
  ...
  // Update branch funding when data loads
  useEffect(() => {
    if (BranchFundingData?.data && BranchFundingData.data.length > 0) {
      const currentBranchFunding = form.getFieldValue(['accounts', 'funding_branch_id']);
      if (!currentBranchFunding) {
        form.setFieldsValue({
          accounts: {
            ...form.getFieldsValue().accounts,
            funding_branch_id: BranchFundingData.data[0].key
          }
        });
      }
    }
  }, [BranchFundingData?.data, form]);
...
try {
    ...
    if (payload.min_principal_amount > payload.max_principal_amount) {
    ...
    setLoading(true);
     postRequest(URL_ADD_LOAN_PRODUCT,{...payload},jwt)
     .then((res) => {
      setLoading(false);
       form.resetFields();
       setConfiguredFees([]);
  ...
  handleRequestError(error);
    ...
    // setTimeout(() => {
    //   setLoading(false);
    //   message.success("Loan product created successfully (demo)");
    //   form.resetFields();
    //   setConfiguredFees([]);
    //   setLatePenaltyEnabled(true);
    //   setDurationType('fixed');
      ...
      <Card
        title={
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 24, fontWeight: 700, color: "#2a3f54" }}>Add a Loan Product</span>
      ...
      >
        <Form
          ...
          initialValues={{
            ...
            accounts: {
              funding_account: CashFundingAccountsData?.data?.[0]?.id || "",
              funding_branch_id: "",
              loans_receivable_account: CoaForLoansData?.data?.find(acc => acc.acc_name === "Loans Receivable")?.id || CoaForLoansData?.data?.[0]?.id || "",
              interest_income_account: CoaForLoansData?.data?.find(acc => acc.acc_name === "Interest Income")?.id || CoaForLoansData?.data?.[0]?.id || "",
              fees_income_account: CoaForLoansData?.data?.find(acc => acc.acc_name === "Loan Fee Income")?.id || CoaForLoansData?.data?.[0]?.id || "",
              penalty_income_account: CoaForLoansData?.data?.find(acc => acc.acc_name === "Penalty Income")?.id || CoaForLoansData?.data?.[0]?.id || "",
              overpayment_account: CoaForLoansData?.data?.find(acc => acc.acc_name === "Overpayment")?.id || CoaForLoansData?.data?.[0]?.id || ""
            }
          }}
        >
