...
import { URL_UPDATE_LOAN_ACCOUNT, URL_GET_CASH_FUNDING_ACCOUNTS, URL_GET_COA_FOR_LOANS, URL_GET_FUNDING_BRANCHES } from "@/config/api-paths";
import useHandleResponse from "@/hooks/useHandleResponse";
import useFetchQuery from "@/hooks/ReactQuery/useFetchQuery";
import { BUTTON_CONFIGS } from "@/utils/buttonStyles";
const { Option } = Select;

const LoanAccountModal = (props) => {
  let jwt = props?.jwt;
  ...
  // console.log("BranchFundingData.data", BranchFundingData?.data);
  // console.log("BranchFundingData.isLoading", BranchFundingData?.isLoading);
  // console.log("BranchFundingData.isError", BranchFundingData?.isError);
  // console.log("URL_GET_FUNDING_BRANCHES", URL_GET_FUNDING_BRANCHES);
  // console.log("selectedFundingAccount", selectedFundingAccount);
  // console.log("Branch funding URL:", selectedFundingAccount ? `${URL_GET_FUNDING_BRANCHES}/${selectedFundingAccount}` : null);
  ...
  const onFinish = async values => {
    ...
    try {
      ...
      await updateRequest(URL_UPDATE_LOAN_ACCOUNT,record.loan_id, {...data}, jwt)
      .then((res) => {
      ...
      <Card
        title={
          isDataLoading ? (
            ...
            <span style={{ fontSize: 20, fontWeight: 700, color: "#2a3f54" }}>Edit Loan Accounts</span>
          )
        }
        bordered={false}
